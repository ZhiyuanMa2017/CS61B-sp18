...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        if (inBounds(left) && min(index, left) == left) {
            if (inBounds(right) && min(left, right) == right) {
                swap(index, right);
                sink(right);
            } else {
                swap(index, left);
        ...
        if (inBounds(right) && min(index, right) == right) {
            swap(index, right);
            sink(right);
        }
    }
